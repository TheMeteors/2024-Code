...
if scrape == 'e':
    ...
    web_link = f"https://theorangealliance.org/events/"
...
def scrape_team_numbers(url):
    ...
    html = response.text  # Getting the html code
    ...
    for team in team_elements:
        ...
        if team_text.isdigit() and team_text not in team_numbers:  # Make sure that it is a number
...
def get_team_data(team_number, season_key, include_long_name):
    try:
        ...
        # Include the ong name if wanted
...
def collect_and_save_data(team_numbers, season_key, include_long_name, filename):
    ...
    with open(filename, mode='w', newline='') as file:
        ...
        # Write data for each of the team
...
# Scrape for the team numbers or get them manually
if scrape == 'e':
    yes = input('Would you like to get data from matches or teams (m/t)')
    if yes == 'm':
        getter(link)
        sys.exit()
